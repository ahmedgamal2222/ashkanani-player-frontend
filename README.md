...
- **`#presence` — الحضور عالميًا** (`components/sections/presence.tsx`): خريطة عالم **حقيقية** بإسقاط متساوي المستطيلات (2:1) من ويكيميديا كومنز، بإطلالة **أطلس حديثة** (محيط كحلي متدرج + يابسة رمادية-سماوية بحافة مضيئة عبر `mask-image`)، مع:
  - **مسح رادار** يدور حول نقطة اللاعب، وشبكة خطوط طول/عرض.
  - **تفاعل كامل**: مرّر أو اضغط أي سوق على الخريطة أو من قائمة الأسواق → يتوهج السوق وقوسه، وتظهر بطاقة تفاصيل بمعلوماته و**المسافة الجوية الحقيقية من الكويت** (معادلة هافرساين).
  - **أزرار تكبير/تصغير/إعادة** (1× → 3.5×) مثبّتة على نقطة الكويت فيبقى الدبوس في مكانه.
  - **علم الكويت الرسمي** (`components/kuwait-flag.tsx`) كشارة على الدبوس وفي بطاقة مستقلة مع الإحداثيات.
  - إحداثيات الدبوس: `x = خط الطول + 180`، `y = 90 − خط العرض` (الكويت: 29.3759N / 47.9774E). لو تعذّر تحميل صورة الخريطة تبقى الشبكة والأقواس والدبوس ظاهرة بدون كسر التصميم.
...
- **الهيرو**: اسم اللاعب بمقاس سائل `clamp(2rem, 8.5vw, 6rem)` مع `break-words` و`text-wrap: balance` فلا يُقطع على أي جهاز؛ بطاقة اللاعب على الجوال بنسبة `4/5` (وأفقية `3/4` من `sm`) مع `min-w-0` + `truncate` للنصوص وشارة رقم أصغر — لا اقتطاع للصورة أو النصوص.
- **صورة اللاعب داخل الملعب**: دائرة ذهبية بصورته (مكبّرة، مثبتة من الأعلى `xMidYMin`) + شارة الرقم `clipPath` على `<image>`، مع **بطاقة صورة اللاعب** داخل ركن الملعب تظهر على كل المقاسات.
