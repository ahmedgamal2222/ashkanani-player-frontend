...
- **اسم اللاعب في الناف بار**: شعار الوكالة (`AgencyLogo` — بارتفاع 36px على الجوال و44px من `sm`، **بلا إطار ذهبي وبلا خلفية**) + صورة اللاعب الدائرية (تظهر من `sm` فقط لتجنّب الزحام) + الاسم (يُقطع بـ`truncate` ولا يكسر التصميم)، والنص التعريفي للوكالة يظهر من `xl` فقط.
