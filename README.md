...
- **شعار الوكالة** (`components/agency-logo.tsx` + `public/images/logo.png`): مكوّن `AgencyLogo` موحّد يُستخدم في الناف بار والهيرو والفوتر وقسم التواصل وبطاقات الملف — لوح بحدّ ذهبي وخلفية عاجية (تندمج مع خلفية الشعار الفاتحة فيظهر التاج وحروف ASM بتفاصيلها)، مع **لمعان ذهبي يمرّ** (`brand-sheen`) و**هالة نابضة** (`halo`) وظلال ذهبية. المقاسات الجاهزة: `xs` (الناف بار) → `xl` (الهيرو)، والتحكّم النهائي بالارتفاع عبر `className` (مثال: `h-20 sm:h-24 lg:h-28`).
...
- **اسم اللاعب في الناف بار**: لوح شعار الوكالة (`AgencyLogo` — كبير 40px على الجوال و48px من `sm`) + صورة اللاعب الدائرية (تظهر من `sm` فقط لتجنّب الزحام) + الاسم (يُقطع بـ`truncate` ولا يكسر التصميم)، والنص التعريفي للوكالة يظهر من `xl` فقط.
